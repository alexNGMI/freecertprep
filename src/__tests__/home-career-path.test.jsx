...
describe('homepage career-path layout', () => {
  ...
  it('presents the IT career paths while keeping the full catalog visible', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Choose a direction. Build exam confidence.' })).toBeTruthy()
    ...
    expect(screen.getByRole('heading', { name: 'Build the network layer' })).toBeTruthy()
    ...
    expect(screen.getAllByRole('link', { name: /Open path/ })[0].getAttribute('href')).toBe('/paths/it-entry')
    expect(screen.getAllByRole('link', { name: /Open path/ })[1].getAttribute('href')).toBe('/paths/networking')
    expect(screen.getAllByRole('link', { name: /Open path/ })[2].getAttribute('href')).toBe('/paths/cybersecurity')
    expect(screen.getAllByRole('link', { name: /Open path/ })[3].getAttribute('href')).toBe('/paths/cloud')
    expect(screen.getAllByRole('link', { name: /Open path/ })[4].getAttribute('href')).toBe('/paths/nvidia')
