...
describe('A+ selection page', () => {
  ...
  it('keeps Core 1 and Core 2 separate from the catalog grid', () => {
    ...
    expect(screen.getByText('Core 1 live questions')).toBeTruthy()
    expect(screen.getByText('PBQ-lite scenarios')).toBeTruthy()
    expect(screen.getByText('Live foundational practice · C+ · breadth expansion queued')).toBeTruthy()
    expect(screen.getByText(/objective breadth and distractor quality continue to expand/i)).toBeTruthy()
  ...
  it('switches the visible blueprint when a core is selected', () => {
    ...
    expect(screen.getByText('760')).toBeTruthy()
    expect(screen.getByText('10')).toBeTruthy()
    expect(screen.getByText('Core 2 live questions')).toBeTruthy()
    expect(screen.getByText('PBQ-lite scenarios')).toBeTruthy()
    expect(screen.getByText('Live foundational practice · C+ · breadth expansion queued')).toBeTruthy()
