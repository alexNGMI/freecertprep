...
describe('career path pages', () => {
  ...
  it('guides cloud learners through AWS foundation, SAA, and Terraform', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Optional foundation — new to cloud?' })).toBeTruthy()
    expect(screen.getByRole('heading', { name: 'Earn the career credential' })).toBeTruthy()
    expect(screen.getByRole('heading', { name: 'Add an applied skill' })).toBeTruthy()
    expect(screen.getByText('New to cloud? Start with Cloud Practitioner.')).toBeTruthy()
    expect(screen.getByText('Already comfortable with cloud fundamentals? Begin with SAA.')).toBeTruthy()
    expect(screen.getByText('Terraform turns architecture knowledge into a deployable infrastructure skill.')).toBeTruthy()
    expect(screen.getByText('Step 03 / Applied Skill')).toBeTruthy()
    expect(screen.getByText('Cloud Support Associate')).toBeTruthy()
  ...
  it('offers Network+ or CCST as live foundations while holding future CCNA v2.0', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Choose a foundation' })).toBeTruthy()
    expect(screen.getByRole('heading', { name: 'Build toward the career credential' })).toBeTruthy()
    expect(screen.getByText('Choose Network+ for broad vendor-neutral fluency or CCST for a Cisco-first foundation.')).toBeTruthy()
    expect(screen.getByText('CCNA is the career credential this direction builds toward.')).toBeTruthy()
    expect(screen.getByText('Each step builds toward real network troubleshooting and operations skill.')).toBeTruthy()
    expect(screen.getByText('Step 01 / Option A / Foundation')).toBeTruthy()
    expect(screen.getByText('Step 01 / Option B / Foundation')).toBeTruthy()
    expect(screen.getByText('Best default')).toBeTruthy()
    expect(screen.getByText('Cisco route')).toBeTruthy()
    expect(screen.getByText('Open CompTIA Network+')).toBeTruthy()
    expect(screen.getByText('Open Cisco CCST Networking')).toBeTruthy()
