...
- **Path pages** - Dedicated lanes for A+ entry, Networking, Cybersecurity, Cloud, and NVIDIA. Networking now frames Network+ or CCST Networking as level-one options before CCNA while CCNA remains preview-only; Cybersecurity keeps Network+ into Security+. Cloud guides learners through AWS Cloud Practitioner, SAA, then Terraform. A+ Core 1 and Core 2, Azure Fundamentals, and Google CDL remain in the full catalog for direct entry.
