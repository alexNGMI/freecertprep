...
- **Path pages** - Dedicated lanes for A+ entry, Cisco networking, Cybersecurity, Cloud, and NVIDIA. Networking now frames CCST into CCNA while CCNA remains preview-only; Cybersecurity keeps Network+ into Security+. Cloud guides learners through AWS Cloud Practitioner, SAA, then Terraform. A+ Core 1 and Core 2, Azure Fundamentals, and Google CDL remain in the full catalog for direct entry.
