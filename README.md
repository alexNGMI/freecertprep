...
- **Current roadmap** - The catalog remains frozen. Cloudflare hosting, Supabase email authentication, manual account backup/restore, signed-in question reports, and the protected admin review workspace are implemented. The admin queue requires the June 24 Supabase migration and first-admin promotion before production use. Next platform priorities are merge-aware sync, domain support/admin email, and privacy/data-management hardening. See `docs/current-state-and-next-steps-2026-06-13.md`, `docs/backend-accounts-architecture-2026-06-17.md`, and `docs/admin-report-review-runbook.md`.
...
- **Backend/accounts status** - The app is live on Cloudflare Workers Static Assets. Supabase magic-link sign-in, sign-out, manual full-study snapshot backup/restore, signed-in issue-report persistence, and a private `/admin/reports` review workspace are implemented. Anonymous study remains fully available. Automatic background sync, conflict merging, domain email, and account privacy controls remain next.
...
- Tailwind CSS v4
- React Router v7
- Vitest (1,305 tests across 46 files), GitHub Actions CI
